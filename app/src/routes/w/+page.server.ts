...
export const actions: Actions = {
    createWorkspace: async({request, locals}) => {
        ...
        try {
         const record = await locals.pb.collection('workspaces').create({name: form.data.name, description: form.data.description, end_of_the_year: new Date(form.data.end_of_the_year), beginning_of_the_year: new Date(form.data.beginning_of_the_year)})
