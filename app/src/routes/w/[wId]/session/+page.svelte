...
<section>
	<div class="title">
		...
		<small
			>{workspace.beginning_of_the_year.split(' ')[0]} &nbsp; {workspace.end_of_the_year.split(
				' '
			)[0]}
		</small>
	</div>
	...
	<div class="container">
					...
					{#each allSession as session}
						<a href="/w/{$page.params.wId}/session/{session.id}">
							<div>
								...
								<small>
									{getDateOnly(session.beginning_of_the_session)} &nbsp; {getDateOnly(
										session.end_of_the_session
									)}
								</small>
							</div>
						</a>
		...
		{:else}
			<h2>your workspace is currently empty</h2>
			<div class="create-f-board">
				<Button size="lg" on:click={() => (open = true)}>Create your first session</Button>
				<Modal
					passiveModal
					bind:open
					modalHeading="Create sessionn"
					on:open
					on:close
					style="flex-grow: 1"
				>
					<form use:enhance action="?/createSession" method="POST">
						<DatePicker
							bind:valueFrom={$form.beginning_of_the_session}
							bind:valueTo={$form.end_of_the_session}
						...
						<Button size="field" type="submit" style="flex-grow: 1;">Create</Button>
					</form>
				</Modal>
			</div>
		{/if}
