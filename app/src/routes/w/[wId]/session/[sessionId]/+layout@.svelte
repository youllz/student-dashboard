<script lang="ts">
	import { Header, SkipToContent, HeaderNav, HeaderNavItem, HeaderNavMenu,  Breadcrumb, BreadcrumbItem, TooltipDefinition, Popover, Button, ButtonSet, TextInput, TextArea,  Tabs, Tab, TabContent, HeaderUtilities, HeaderGlobalAction } from 'carbon-components-svelte';
    import type { LayoutData } from './$types';
	import { TrashCan, Close, Add, SettingsAdjust, OpenPanelFilledLeft, SidePanelClose } from 'carbon-icons-svelte';
...
<Header  bind:isSideNavOpen>
    <Button kind="ghost" iconDescription={$panelIsOpen? 'close panel' : 'open panel'} icon={$panelIsOpen? SidePanelClose : OpenPanelFilledLeft} on:click={() => $panelIsOpen = !$panelIsOpen} />
     ...
     <aside>
                     ...
                     <div class="content">
                         <form use:enhance action="?/createMatter" method="POST">
                             <div>
                                 ...
                                 <TextInput bind:value={$form.name} type="text" name="name" id="name" placeholder="Matter name" required autocomplete="off"/>
                             </div>
                             <div>
                                 <label for="description">Matter name</label>
                                 <TextArea  bind:value={$form.description} name="description" id="description" placeholder="Matter description" autocomplete="off"/>
                             </div>
                             
                             <div class="footer">
                                 <ButtonSet >
                                     <Button  on:click={() => (open = !open)} type="button" size="field" kind="secondary">Cancel</Button>
         ...
         <div class="matters-container">
             <ul>
                 ...
                 <li class:focus={$page.url.pathname === `/w/${$page.params.wId}/session/${$page.params.sessionId}/matter/${matter.id}`}>
