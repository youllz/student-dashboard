							...
							<Tile>
								<div class="title">
									...
									<Button
										size="small"
										kind="tertiary"
										iconDescription="add new workspace"
										icon={Add}
										on:click={() => (open = true)}
									/>
										...
										<div class="add-w">
											<form use:enhance action="/w?/createWorkspace" method="POST">
												...
												<DatePicker
													...
													bind:valueFrom={$form.beginning_of_the_year}
													bind:valueTo={$form.end_of_the_year}
													on:change
												>
													<DatePickerInput
							...
							<Accordion size="sm">
								{#each allWorksapcesData as item}
									<AccordionItem title={item.name}>
										<Button
											kind={$page.url.pathname === `/w/${item.id}/session` ? 'secondary' : 'ghost'}
											size="field"
											icon={CicsSystemGroup}
											href="/w/{item.id}/session"
											style="width: 100%;"
										>
											<span>session</span>
										</Button>
